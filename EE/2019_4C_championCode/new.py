...
def hough_lines(img, rho, theta, threshold, min_line_len, max_line_gap):
    ...
    try:
        for line in lines:
            for x1,y1,x2,y2 in line:
                if (abs(y1-y2)< 20)or(abs(x2-x1)< 20):
                    continue
##############################################################
                k = float(y2-y1)/(x2-x1)
                #print('x2:',x2,'y2:',y2)
    ...
    if(lines4[0]!=[]):
        for line in lines4:
            ...
            if(abs(x2-x1)>10):
                ...
                if(k3>0): #
                    ...
                    #print("right")
                    #b1=y1-x1*k3#b=y-kx
                    #right_x=int((279-b1)/k)#x=(y-b)/k
                    if(x1>640):
                        right_x=x1-640
                        right_find=1
                        #print("right_find",right_find)
                    #print("b.right_y2", b.right_y2)
                    #b.Print()
                    #left_y=y1-x1*k3+k3*160#b=y-kx
                else:
                    b.left_x1=x1
                    b.left_y1=y1
                    b.left_x2=x2
        ...
        #mid_x=abs(mid_x)
        #mid_x=int((mid_x/4)*255/600)
        #print("mid_x",mid_x)
        
    draw_lines(line_img, [lines4], thickness=20)
    if(find_flag!=0):
        
        if(right_find+left_find)==2:
            turn=0x03
        elif right_find==1:
            ...
            turn=0x03 #  
        print("find")
    if(find_flag==0):#未找到 发送历史值并判断是否为虚线段
        print("no find")
        b.is_xuxian()
...
while(1):
    ...
    img = frame[200:479,0:639] #img[120:430,100:450]  # 裁剪坐标为[y0:y1, x0:x1]
    #img = img(Range(0,100));
